...
$search_date_relance = GETPOST('search_date_relance', 'alpha');
...
$arrayfields = array(
	...
	's.barcode'=>array('label'=>"Gencod", 'position'=>5, 'checked'=>0, 'enabled'=>(!empty($conf->barcode->enabled))),
	...
	's2.nom'=>array('label'=>'ParentCompany', 'position'=>64, 'checked'=>1),
...
if (!empty($conf->global->PRODUIT_MULTIPRICES) || !empty($conf->global->PRODUIT_CUSTOMER_PRICES_BY_QTY_MULTIPRICES)) {
	$arrayfields['s.price_level'] =array('label'=>"PriceLevel", 'position'=>30, 'checked'=>1);
}
...
$sql .= " pr.comment, pr.date_relance, pr.tms as lastUpdate";
...
$sql .= ' LEFT JOIN '.MAIN_DB_PREFIX."prospection as pr ON pr.fk_soc = s.rowid";
...
if ($search_comment != '') {
	$sql .= natural_search("pr.comment", $search_comment);
}
if ($search_date_relance != '') {
	$sql .= natural_search("pr.date_relance", $search_date_relance);
}
...
if($sortfield=="pr.date_relance")
	$sql .= "ORDER BY CASE WHEN date_format(pr.date_relance, '%d/%m/%Y') < date_format(NOW(), '%d/%m/%Y') OR pr.date_relance IS NULL THEN 1 ELSE 0 END ".$sortorder.", pr.date_relance ".$sortorder;
else
...
if ($search_commant != '') {
	$param .= '&search_comment='.urlencode($search_comment);
}
if ($search_date_relance != '') {
	$param .= '&search_date_relance='.urlencode($search_date_relance);
}
if ($search_parent_name != '') {
...
print '<input class="flat searchstring maxwidth50imp" type="text" name="search_comment" value="'.dol_escape_htmltag($search_comment).'">';
print '</td>';
print '<td class="liste_titre maxwidthonsmartphone right">';
print '<input class="flat searchstring maxwidth50imp" type="text" name="search_date_relance" value="'.dol_escape_htmltag($search_date_relance).'">';
print '</td>';
...
print_liste_field_titre("Commentaire", $_SERVER["PHP_SELF"], "pr.comment", "", $param, '', $sortfield, $sortorder, 'left ');
print_liste_field_titre("Date de relance", $_SERVER["PHP_SELF"], "pr.date_relance", "", $param, '', $sortfield, $sortorder, 'right ');
if (!empty($arrayfields['s2.nom']['checked'])) {
...
require_once DOL_DOCUMENT_ROOT.'/custom/prospection/class/prospection.class.php';
$prospectionETY = new Prospection($db);
$prospectionStatus = $prospectionETY->getProspectionStatus();
...
while ($i < min($num, $limit)) {
	...
	if (!empty($arrayfields['s.fk_stcomm']['checked'])) {
		// Prospect status
		print '<td id="fk_stcomm'.$companystatic->id.'" class="center nowrap"><div class="nowrap">';
		print '<div class="inline-block">';
		$prospectionETY->fk_soc = $companystatic->id;
		print $prospectionETY->selectProspectionStatus($obj->stcomm_id, $prospectionStatus);
		print '</div></div></td>';
		if (!$i) {
			$totalarray['nbfield']++;
		}
	...
	// Prospect comment
	$dateLastUpdate = date_create_from_format('Y-m-d H:i:s', $obj->lastUpdate);
	$dateLastUpdate = date_format($dateLastUpdate, 'd/m/Y H:i');
	print '<td id="comment'.$companystatic->id.'" class="left nowrap" style="vertical-align:middle;"><div class="nowrap">';
	print '<div class="inline-block">';
	print '<textarea style="min-width:165px;min-height:45px;max-height:150px;max-width:500px;" maxlength=250 class="fillable" data-id="'.$companystatic->id.'" data-field="comment" name="comment">'.$obj->comment.'</textarea>';
	print '</div>';
	print '<span data-id="'.$companystatic->id.'" class="fa fa-info-circle more-info"style="font-size:20px; position:absolute;margin-top:0px;"></span>&nbsp;&nbsp; ';
	print '</div>Dern. màj : '.$dateLastUpdate.'</td>';
	if (!$i) {
		$totalarray['nbfield']++;
	}

	// Prospect relance status
	print '<td id="date_relance'.$companystatic->id.'" class="right nowrap"><div class="nowrap">';
	$prospectionETY->fk_soc = $companystatic->id;
	$dateRelance = date_create_from_format('Y-m-d H:i:s', $obj->date_relance);
	$dateRelance = date_format($dateRelance, 'Y-m-d');
	if($dateRelance==date('Y-m-d')){
		print '<span class="fa fa-warning "style="color:red; font-size:20px;"></span>&nbsp;&nbsp; ';
	}
	print '<input type="date" class="fillable" data-id="'.$companystatic->id.'" data-field="date_relance" name="date_relance" value="'.$dateRelance.'">';
	print '</div></td>';
	if (!$i) {
		$totalarray['nbfield']++;
	}
	...
	print '<input id="add-button'.$companystatic->id.'" type="button" class="flat add-button" data-id="'.$companystatic->id.'" value="+">';
...
?>
	...
	<script>
		$(".select-prospection-status").select2({
				placeholder:"Statut de prospection",
				clear:true
			...
			if (index)
				$.ajax({
					...
					success: function(data) {
						console.log(data);
						if(data.includes('ok')){
							$('#fk_stcomm'+index).css('background', 'lightgreen');
						}
						if(data.includes('ko')){
							$('#fk_stcomm'+index).css('background', 'orange');
						}
					}
				});
		});
		...
		for (fill of fillable) {
			$(fill).on('focusout', function(event) {
				...
				if (this.value && index)
					$.ajax({
						...
						success: function(data) {
							console.log(data);
							if(data.includes('ok')){
								$('#'+field+index).css('background', 'lightgreen');
							}
							if(data.includes('ko')){
								$('#'+field+index).css('background', 'orange');
							}
						}
					});
			});
				...
				if (index)
					$.ajax({
						...
						success: function(data) {
							console.log(data);
							if(data.includes('ok')){
								$("#add-button"+index).css('border', '2px solid green');
							}
							if(data.includes('ko')){
								$("#add-button"+index).css('border', '2px solid red');
							}
						}
					});
			});
		}
		var moreInfos = $('.more-info');
		for (moreInfo of moreInfos) {
					...
					$.ajax({
						...
						success: function(data) {
							if(data.includes('ko')){
								...
								$("#dialog").html(data);
								$("#dialog").dialog({
									title: 'Liste des 20 derniers événements',
									maxHeight: 500,
									width: 1200
								});
							}
